...
type ClipSide = 'left' | 'right' | 'top' | 'bottom';
...
interface FusionClipCardProps {
  service: FusionService;
  provider: 'YottaFlex' | 'Inforica';
  tags: string[];
  className?: string;
...
const ENTRANCE_KEYFRAMES: Record<ClipSide, string[]> = {
...
const EXIT_KEYFRAMES: Record<ClipSide, string[]> = {
...
function FusionClipCard({
  ...
  const isInforica = provider === 'Inforica';

  const getNearestSide = (event: React.MouseEvent<HTMLDivElement>): ClipSide => {
    const box = event.currentTarget.getBoundingClientRect();
    const sortedProximity = [
      { proximity: Math.abs(box.left - event.clientX), side: 'left' as const },
      { proximity: Math.abs(box.right - event.clientX), side: 'right' as const },
      { proximity: Math.abs(box.top - event.clientY), side: 'top' as const },
      { proximity: Math.abs(box.bottom - event.clientY), side: 'bottom' as const },
  ...
  const handleMouseEnter = (event: React.MouseEvent<HTMLDivElement>) => {
    ...
    animate(scope.current, { clipPath: ENTRANCE_KEYFRAMES[side] }, { duration: 0.35, ease: 'easeOut' });
  };

  const handleMouseLeave = (event: React.MouseEvent<HTMLDivElement>) => {
    const side = getNearestSide(event);
    animate(scope.current, { clipPath: EXIT_KEYFRAMES[side] }, { duration: 0.3, ease: 'easeInOut' });
  };

  return (
    <div
      className={`fusion-card fusion-clip-card ${className}`}
    ...
    >
      <div className="fusion-card-base">
        <div className="fusion-card-top">
          <span className={`fusion-provider ${isInforica ? 'provider-inforica' : 'provider-yotta'}`}>{provider}</span>
          <div className="fusion-card-icon">
        ...
        <h4 className="fusion-card-title">{service.title}</h4>
        <p className="fusion-card-desc">{service.desc}</p>
        <div className="fusion-card-bottom">
          {tags.map((tag) => (
            <span key={tag} className="fusion-tech-tag">{tag}</span>
          ))}
        </div>
      </div>
...
export default function PartnershipPage({ onWaitlistClick }: PartnershipPageProps) {
  ...
  return (
    <div className="partnership-page-wrapper" style={{ color: 'var(--text-primary)', position: 'relative', zIndex: 1 }}>
      ...
      {/* TAB CONTENT 2: COMBINED SERVICES — FUSION LAYOUT */}
      {activeTab === 'services' && (
        <div key="services-tab" className="fusion-services-wrapper">
          
          <section className="fusion-services-section">
            <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
              
              {/* ── Header ── */}
              <div className="fusion-header" data-aos="fade-up">
                <span className="accent-label">Shared Capabilities</span>
                <h2>Our Combined Services Portfolio</h2>
                <p>
              ...
              {/* ── Bridge Visualizer ── */}
              <div className="fusion-bridge" data-aos="fade-up" data-aos-delay="100">
                <div className="fusion-bridge-inner">
                  {/* left platform */}
                  <div className="fusion-platform yotta-platform">
                    <div className="fusion-platform-glow"></div>
                    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                      <polygon points="12 2 2 7 12 12 22 7 12 2" />
                      <polyline points="2 17 12 22 22 17" />
                      <polyline points="2 12 12 17 22 12" />
                    </svg>
                    <span className="fusion-platform-label">YottaFlex</span>
                    <span className="fusion-platform-sub">AWS Engineering</span>
                  </div>

                  {/* bridge with particles */}
                  <div className="fusion-connector">
                    <div className="fusion-connector-line"></div>
                    <div className="fusion-particle p1"></div>
                    <div className="fusion-particle p2"></div>
                    <div className="fusion-particle p3"></div>
                    <div className="fusion-particle p4"></div>
                    <div className="fusion-bridge-center">
                  ...
                  {/* right platform */}
                  <div className="fusion-platform inforica-platform">
                    <div className="fusion-platform-glow"></div>
                    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    ...
                    <span className="fusion-platform-label">Inforica</span>
                    <span className="fusion-platform-sub">Azure Enterprise</span>
                  </div>
                </div>
              </div>
              ...
              <div className="fusion-cards-grid">
                ...
                {inforicaServices.map((service, i) => (
                  <FusionClipCard
                    key={`inf-${i}`}
                    className="inforica-card"
                    service={service}
                    provider="Inforica"
                    tags={['Azure', 'Microsoft', 'Security']}
                    animationDelay={`${(i + yottaServices.length) * 0.08}s`}
                    aosDelay={(i + yottaServices.length) * 50}
                  />
                ))}
              </div>

              {/* ── Equation Banner ── */}
              <div className="fusion-equation" data-aos="fade-up" data-aos-delay="500">
                <div className="fusion-equation-glow"></div>
                <div className="fusion-equation-content">
                  <div className="fusion-equation-formula">
                    <span className="eq-term aws">AWS</span>
                    <span className="eq-plus">+</span>
                    <span className="eq-term azure">Azure</span>
                    <span className="eq-plus">+</span>
                    <span className="eq-term genai">GenAI</span>
                    <span className="eq-equals">=</span>
                    <span className="eq-result">Unified Enterprise Cloud</span>
                  </div>
                  <p>
        ...
        <div key="coverage-tab" data-aos="fade-in">
          ...
          <section className="partnership-map" style={{ padding: '5.5rem 2rem' }}>
            <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
              ...
              <div className="partnership-map-container" style={{
                ...
                <div className="card">
                  <div className="logo1">
                    <svg width="60" height="40" viewBox="0 0 33 22">
                  ...
                  <div className="logo2">UNITED STATES</div>
                ...
                <div className="card">
                  ...
                  <div className="logo2">CANADA</div>
                  <div className="logo-bottom-text">
                ...
                <div className="card">
                  <div className="logo1">
                    <svg width="60" height="40" viewBox="0 0 33 22" fill="none">
                  ...
                  <div className="logo2">GLOBAL DELIVERY</div>
                  <div className="logo-bottom-text">
          ...
          <section className="partnership-industries" style={{ padding: '5.5rem 2rem' }}>
            <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
              ...
              {/* Tag Cloud */}
              <div className="industry-tag-cloud" data-aos="fade-up">
                <div className="industry-tag-track">
                  {[0, 1].map((setIndex) => (
                    <div className="industry-tag-content" key={setIndex}>
